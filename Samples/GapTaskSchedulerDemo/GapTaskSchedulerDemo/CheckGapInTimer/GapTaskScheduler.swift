...
public class GapTaskScheduler {
    ...
    public func schedule(_ block: @escaping GapTaskSchedulerBlock) {
        ...
        // Execute `block` immediately if the gap between now and `lastExecutionDate` is equal or greater than `gap`
        if !executeBlockImmediately(block) {
     ...
     */
    @objc public func timerTick(_ timer: Timer) {
        ...
        executeBlockImmediately(executionBlock)
    }
}
...
fileprivate extension GapTaskScheduler {
    ...
    /**
     Actual execute `block` immediately
     */
    @discardableResult
    func executeBlockImmediately(_ block: @escaping GapTaskSchedulerBlock) -> Bool {
        guard Date().timeIntervalSince(lastExecutionDate) >= gap else {
            return false
        }
        // Execute `block` immediately if the gap between now and `lastExecutionDate` is equal or greater than `gap`
